...
const AIChatPanel: React.FC<{ isOpen: boolean; onClose: () => void }> = ({
  ...
  // Load the runtime AI configuration (public/ai-config.json)
  useEffect(() => {
    let cancelled = false;
  ...
  const doSend = async (content: string) => {
    ...
    try {
      const { answer, toolCalls } = await runQwenAgent({
        config: cfg,
        apiKey: getApiKey(),
      ...
      setMessages((prev) => [
        ...prev,
        {
          ...
          content: errMsg.includes("401")
            ? "Authentication failed. Please check the AI credentials configured for this deployment (AI_DSP_USERNAME / AI_DSP_PASSWORD environment variables or public/ai-config.json)."
      ...
      <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
        ...
        >
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
            <EuiToolTip content="AI settings (model / credentials)">
              <EuiButtonIcon
                iconType="gear"
